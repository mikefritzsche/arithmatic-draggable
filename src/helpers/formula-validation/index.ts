...
export function flattenFormula(items: ArithmeticFormulaElement[], parent = '0') {
  const flat: ArithmeticFormulaElement[] = []
  ...
  items.forEach(item => {
    if (Array.isArray(item.children)) {
      ...
      el.parentId = parent
      flat.push(el)
      flat.push(...flattenFormula(item.children, item.id))
    }
    else {
