<template>
  <div>
    ...
    <div class="flex" style="gap: 10px; justify-content: center; align-items: center">
      <pre class="title">original:</pre><pre>{{ formulaStringBase }}</pre>
      <pre class="title">Modified:</pre><pre>{{ formulaString }}</pre>
    </div>
...
export default {
  ...
  methods: {
    ...
    removeContainingBlock() {
      const length = this.formula.length
      const formula = cloneDeep(this.formula)
      const parentId = formula[0].id

      formula.forEach((item, i) => {
        if (i !== 0 && i !== formula.length - 1 && item.parentId === parentId) {
          item.parentId = '0'
        }
        return item
      })
